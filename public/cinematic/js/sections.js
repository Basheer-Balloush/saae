...
(() => {
  ...
  function autoplay(section, delay, advance) {
    ...
    const api = {
      ...
      pause(ms) {
        api.hold();
        window.setTimeout(api.release, ms);
      },
    ...
    section.addEventListener("pointerleave", api.release);
    section.addEventListener("focusin", api.hold);
    section.addEventListener("focusout", api.release);
