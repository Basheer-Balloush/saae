...
export function initTextEffect(): () => void {
 ...
 *
  ...
  */
  ...
  const startRectFallback = () => {
    ...
    const queue = () => { if (!frame) frame = __raf(measure); };
    ...
    /* Scroll drives the responsive path; the interval is insurance. If this
       ...
       hidden for good. */
    const tick = __setInterval(measure, 300);
    ...
    __on(window, "scroll", queue, { passive: true });
    __on(window, "resize", queue, { passive: true });
    measure();
  };
  ...
  const build = () => {
    ...
    let proved = false;
    observer = new __IntersectionObserver(entries => {
    ...
    const verifyTimer = __setInterval(() => {
  ...
  const waitForOpening = () => {
    ...
    const classWatcher = new __MutationObserver(() => {
    ...
    // gets its text.
    __setTimeout(start, 6000);
  };

  /* language.js swaps text node values under us. Give it the original nodes
     back, let it translate, then split the new copy. */
  __on(window, "saae:languagechange", () => {
    if (!started) return;
    teardown();
    __setTimeout(build, 0);
  });

  __on(reducedMotion, "change", () => {
    if (started) build();
  });

  if (document.readyState === "loading") {
    __on(document, "DOMContentLoaded", waitForOpening);
  } else {
    waitForOpening();
  }

  };
  try { __run(); } catch (error) { console.error("initTextEffect failed", error); }
  return __teardown;
}
