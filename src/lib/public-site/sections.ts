...
(() => {
  ...
  function visibility(element, onShow, onHide) {
    ...
    new __IntersectionObserver(entries => {
  ...
  if (newsFlow && newsTrack && newsTrack.children.length) {
    ...
    function goTo(next) {
      if (live && scrub) {
        window.scrollTo({ top: scrub.offsetFor(clamp(next, 0, last) / last), behavior: "smooth" });
        return;
      }
      took();
      go(next);
  ...
  if ("IntersectionObserver" in window) {
    const textObserver = new __IntersectionObserver(entries => {
      entries.forEach(entry => {
  ...
  try { __run(); } catch (error) { console.error("initSections failed", error); }
  return __teardown;
}
