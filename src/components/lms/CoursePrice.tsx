 ...
 */
export function CoursePrice({ price, salePrice, isFree, lang, freeLabel, className, size = "lg" }: Props) {
  ...
  return (
    <span className={`inline-flex flex-wrap items-center gap-2 ${className ?? ""}`}>
      {hasSale && (
        <span className={`text-muted-foreground line-through ${size === "sm" ? "text-[11px]" : "text-base font-medium"}`}>
