...
function rowsToDrafts(rows: string[][], ar: boolean): DraftQuestion[] {
  ...
  return body.map((r) => {
    ...
    else if (idx < 0 || !choices[idx]) error = ar ? "الإجابة الصحيحة غير صالحة" : "Invalid correct answer";
...
export function QuizBulkImportDialog({
  ...
  const downloadTemplate = async () => {
    ...
    ws.columns = [{ width: 60 }, { width: 24 }, { width: 24 }, { width: 24 }, { width: 24 }, { width: 12 }];
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : ar ? "تعذّر قراءة الملف" : "Could not read the file");
    } finally {
      setParsing(false);
    }
  };
      ...
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto" dir={ar ? "rtl" : "ltr"}>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <input
            ...
            onChange={handleFile}
          />
          <Button type="button" size="sm" onClick={() => inputRef.current?.click()} disabled={parsing}>
            {parsing ? <Loader2 className="h-4 w-4 animate-spin mx-1" /> : <Upload className="h-4 w-4 mx-1" />}
            ...
            >
              <div className="flex items-start gap-2">
                ...
                <Textarea
                  ...
                  className="flex-1"
                />
                <Button size="sm" variant="ghost" onClick={() => setDrafts(drafts.filter((_, x) => x !== i))}>
        ...
        <DialogFooter className="gap-2">
          ...
          <Button onClick={save} disabled={saving || valid.length === 0}>
            {saving ? <Loader2 className="h-4 w-4 animate-spin mx-1" /> : <Plus className="h-4 w-4 mx-1" />}
