...
export default function TrainerScoringPanel({
  ...
  const [loadedScores, setLoadedScores] = useState<Record<
    string,
    { points: string; comment: string }
  > | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    const [c, ev, sc, sum, inst] = await Promise.all([
      supabase
        .from("trainer_app_criteria")
        .select("*")
        .eq("active", true)
        .order("phase")
        .order("display_order"),
      supabase
        .from("trainer_app_evaluators")
        .select("evaluator_id, assigned_at")
        .eq("application_id", applicationId),
      supabase
        .from("trainer_app_scores")
        .select("criterion_id, points, comment, evaluator_id")
        .eq("application_id", applicationId),
    ...
    setCandidates(
      ((inst.data as { user_id: string; full_name: string }[]) ?? []).filter(
        (i) => i.user_id !== applicantId,
      ),
    );
    setNames(
      Object.fromEntries(
        ((inst.data as { user_id: string; full_name: string }[]) ?? []).map((i) => [
          i.user_id,
          i.full_name,
        ]),
      ),
    ...
    for (const row of (sc.data as {
      criterion_id: string;
      points: number;
      comment: string | null;
      evaluator_id: string;
    }[]) ?? []) {
      if (row.evaluator_id === uid)
        mine[row.criterion_id] = { points: String(row.points), comment: row.comment ?? "" };
    }
    setMyScores(mine);
  ...
  const saveScore = async (crit: Criterion) => {
    ...
    if (!Number.isFinite(points) || points < 0 || points > crit.max_points) {
      toast.error(
        ar
          ? `الدرجة يجب أن تكون بين 0 و ${crit.max_points}`
          : `Score must be between 0 and ${crit.max_points}`,
      );
      return;
    }
    setSavingId(crit.id);
    ...
    <section className="rounded-xl border border-border p-4 space-y-4">
      ...
      <div className="space-y-2">
        <p className="text-xs text-muted-foreground">
          {ar ? "المُقيّمون المعيّنون" : "Assigned evaluators"}
        </p>
        {evaluators.length === 0 ? (
          ...
          <ul className="flex flex-wrap gap-2">
            {evaluators.map((e) => (
              ...
              >
                ...
                <button
                  type="button"
                  onClick={() => removeEvaluator(e.evaluator_id)}
      ...
      {isEvaluator && (
        <div className="space-y-3">
          <p className="text-xs text-muted-foreground">
            {ar ? "تقييمك (لكل معيار)" : "Your scores (per criterion)"}
          </p>
          {[1, 2, 3, 4].map((phase) => (
                ...
                .map((c) => (
                  ...
                  >
                    <span className="text-sm">
                      ...
                      <span className="text-xs text-muted-foreground">/ {c.max_points}</span>
                    </span>
                    <Input
                      type="number"
                      min={0}
                      ...
                      onChange={(e) =>
                        setMyScores((s) => ({
                          ...s,
                          [c.id]: { points: e.target.value, comment: s[c.id]?.comment ?? "" },
                        }))
                      }
                    />
                    <Button
                    ...
                    <Textarea
                      ...
                      onChange={(e) =>
                        setMyScores((s) => ({
                          ...s,
                          [c.id]: { points: s[c.id]?.points ?? "", comment: e.target.value },
                        }))
                      }
                    />
                  </div>
          ...
          <div className="flex flex-wrap gap-3">
            {[1, 2, 3, 4].map((p) => (
              <span key={p}>
                {ar ? `م${p}` : `P${p}`}:{" "}
                <b>
                  {(summary as unknown as Record<string, number>)[`phase${p}`]?.toFixed?.(1) ?? 0}%
                </b>
              </span>
            ))}
            <span>
              {ar ? "النهائي" : "Final"}: <b>{Number(summary.final_score).toFixed(1)}%</b>
            </span>
          </div>
      ...
      {status === "scoring" && summary && !summary.passed && (
        <ActivationChecklist summary={summary} ar={ar} />
      )}
      {status === "scoring" && (
        <Button onClick={activate} disabled={activating || !summary?.passed}>
          {activating ? (
            ...
            <ShieldCheck className="h-4 w-4 mx-2" />
          )}
...
/** Why "Approve & activate" is still off, one line per rule, with the fix. */
function ActivationChecklist({
  ...
  summary: {
    ...
    evaluator_count: number;
    min_evaluators: number;
    pass_final_score: number | string;
    pass_phase_min: number | string;
  } & Record<string, unknown>;
  ar: boolean;
}) {
  const phaseMin = Number(summary.pass_phase_min);
  const phases = [1, 2, 3, 4].map((p) => Number(summary[`phase${p}`] ?? 0));
  const rows: { ok: boolean; text: string; fix?: string }[] = [
    {
      ok: summary.complete,
      text: ar
        ? `${summary.min_evaluators} مُقيّمين على الأقل، وكلٌّ قيّم كل المعايير (المعيّنون الآن: ${summary.evaluator_count})`
        : `At least ${summary.min_evaluators} evaluators, each scoring every criterion (assigned now: ${summary.evaluator_count})`,
      fix:
        summary.evaluator_count < summary.min_evaluators
          ? ar
            ? `عيّن ${summary.min_evaluators - summary.evaluator_count} مُقيّماً آخر من القائمة أعلاه؛ يدخل كل مُقيّم بحسابه ويضع درجاته.`
            : `Assign ${summary.min_evaluators - summary.evaluator_count} more evaluator(s) above; each one signs in with their own account and enters their scores.`
          : ar
            ? "أحد المُقيّمين لم يكمل كل المعايير بعد."
            : "One of the evaluators hasn't scored every criterion yet.",
    },
    {
      ok: Number(summary.final_score) >= Number(summary.pass_final_score),
      text: ar
        ? `الدرجة النهائية ${summary.pass_final_score}% على الأقل`
        : `Final score at least ${summary.pass_final_score}%`,
    },
    {
      ok: phases.every((v) => v >= phaseMin),
      text: ar ? `كل مرحلة ${phaseMin}% على الأقل` : `Every phase at least ${phaseMin}%`,
    },
  ];
  return (
    <div className="rounded-lg border border-amber-500/40 bg-amber-500/10 p-3 text-sm">
      <p className="mb-2 font-bold">{ar ? "قبل الاعتماد يلزم:" : "Before activating:"}</p>
      <ul className="space-y-1.5">
        {rows.map((r) => (
          <li key={r.text}>
            <span className={r.ok ? "text-emerald-500" : "text-amber-500"}>{r.ok ? "✓" : "○"}</span>{" "}
            {r.text}
            {!r.ok && r.fix && <div className="ms-5 text-xs text-muted-foreground">{r.fix}</div>}
          </li>
        ))}
      </ul>
    </div>
