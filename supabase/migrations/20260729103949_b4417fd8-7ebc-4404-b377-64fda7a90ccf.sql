...
CREATE OR REPLACE FUNCTION public.lms_list_catalog_public(
  ...
  _level text DEFAULT NULL::text,
  _search text DEFAULT NULL::text,
  _price text DEFAULT NULL::text
)
 RETURNS TABLE(id uuid, slug text, title_ar text, title_en text, description_ar text, description_en text, cover_url text, level text, price numeric, is_free boolean, students_count integer, rating_avg numeric, review_count integer, category_id uuid, delivery_mode text, total_count bigint)
 LANGUAGE plpgsql
...
BEGIN
  IF cat_requested THEN
    ...
    -- Unknown slug: controlled empty result rather than the full catalogue.
    IF cat IS NULL THEN
      RETURN;
    END IF;
  END IF;
...
GRANT EXECUTE ON FUNCTION public.lms_list_catalog_public(integer, integer, text, text, text, text) TO anon, authenticated;