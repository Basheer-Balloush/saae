...
DECLARE
  ...
  v_enroll_open boolean; v_max int; v_count int;
BEGIN
  IF v_uid IS NULL THEN RAISE EXCEPTION 'unauthenticated'; END IF;
  SELECT price, is_free, status, enrollment_open, max_students, students_count
    INTO v_price, v_is_free, v_status, v_enroll_open, v_max, v_count
  ...
  IF v_max IS NOT NULL AND v_count >= v_max THEN RAISE EXCEPTION 'course full'; END IF;
