...
BEGIN
  ...
  counted AS (
    SELECT b.*, (SELECT count(*) FROM base)::bigint AS total_count FROM base b
  ),
  sorted AS (
  ...
  SELECT
    ...
    s.submitted_at,
    s.withdrawn_at,
    s.snapshot_full_name::text,
    s.snapshot_email::text,
    s.snapshot_phone::text,
    s.snapshot_organization::text,
    s.assigned_admin,
    (SELECT u.email::text FROM auth.users u WHERE u.id = s.assigned_admin) AS assigned_admin_email,
    (SELECT count(*) FROM public.internship_application_course_snapshots c WHERE c.application_id = s.id)::integer AS courses_count,
    (SELECT count(*) FROM public.internship_application_certificate_snapshots c WHERE c.application_id = s.id)::integer AS certificates_count,
    (SELECT count(*) FROM public.internship_application_notes n WHERE n.application_id = s.id)::integer AS notes_count,
...
REVOKE ALL ON FUNCTION public.admin_list_internship_applications(uuid, text, internship_application_status, uuid, uuid, uuid, timestamptz, timestamptz, text, integer, integer) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.admin_list_internship_applications(uuid, text, internship_application_status, uuid, uuid, uuid, timestamptz, timestamptz, text, integer, integer) FROM anon;
GRANT EXECUTE ON FUNCTION public.admin_list_internship_applications(uuid, text, internship_application_status, uuid, uuid, uuid, timestamptz, timestamptz, text, integer, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.admin_list_internship_applications(uuid, text, internship_application_status, uuid, uuid, uuid, timestamptz, timestamptz, text, integer, integer) TO service_role;